...
const hook = (main) => {
    if (main.app.hookSingleton !== undefined) {
        main.app.hookSingleton.createHook('beforeCreate', 'Configuration', 'description');
        main.app.hookSingleton.createHook('afterCreate', 'Configuration', 'description');

        // default variable changed
        main.app.hookSingleton.createHook('afterUpdate', 'Configuration', 'description');
    } else {
        setTimeout( () => {
            hook(main);
        }, 200);
...
module.exports = function(Configuration) {
    ...
    Configuration.remoteMethod('initialized', {
        ...
        description: 'Returns information if Tower was initialized with encryption key',
