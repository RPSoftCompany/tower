...
module.exports = class Configuration {
  ...
  async initializeSecret(secret) {
    ...
    if (secret.length !== 32) {
      ...
      throw new HttpErrors.BadRequest(
        "Encryption key must have length of 32 characters"
      );
    }
    ...
    if (boot.encryptionCheck === undefined) {
      const pass = this.encryptPassword(secret);
      boot.encryptionCheck = pass;
      await boot.save();
    } else {
      try {
        const password = this.decryptPassword(boot.encryptionCheck);
        if (!password) {
  ...
  async createConfiguration(config, options) {
    ...
    const baseCache = await this.app
      .get("BaseConfigurationInstance")
      .getConfigurationModelFromCache();
    ...
    for (const base of allBases) {
      if (config[base.name] !== undefined) {
        ...
        const hasPermissions = await configModel.validateWritePermissions(
          base.name,
          basesObj[base.name],
          userId
        );
        ...
        if (!hasPermissions) {
          ...
          throw new HttpErrors.BadRequest(
            `No permissions to write to ${config[base.name]} model`
          );
        }
    ...
    this.app.hookSingleton.executeAdvancedHook(
      "beforeCreate",
      "Configuration",
      basesObj
    );
    ...
    for (let i = 0; i < allBases.length; i++) {
      ...
      if (model !== null || config[parent.name] === null) {
        ...
        if (model.options !== null) {
          if (model.options.hasRestrictions) {
            ...
            for (const restriction of model.restrictions) {
              ...
              for (const base of allBases) {
                if (
                  ...
                  restriction[base.name] !== config[base.name]
                ) {
                  match = false;
                }
              }
            ...
            if (anyMatch === false) {
              this.log("debug", "createConfiguration", "FINISHED");
              throw new HttpErrors.BadRequest(
                `${model.name} model restriction forbids ` +
                  "this configuration"
              );
            }
          }
        ...
        this.log("debug", "createConfiguration", "FINISHED");
        throw new HttpErrors.BadRequest(
          `Invalid value for ${parent.name} model`
        );
      }
    }
    ...
    if (allBasesAreNull === true) {
      this.log("debug", "createConfiguration", "FINISHED");
      throw new HttpErrors.BadRequest(
        "At least one Base Model needs to be filled in"
      );
    }
          ...
          if (regex.test(variable[rule.targetType])) {
            if (rule.conditionRegEx) {
              ...
              if (!conRegEx.test(`${variable[rule.conditionType]}`)) {
                this.log("debug", "createConfiguration", "FINISHED");
                throw new HttpErrors.BadRequest(
                  `Invalid ${rule.conditionType} in ${variable.name}: ${rule.error}`
                );
              }
            } else {
    ...
    config.variables.map((variable, i) => {
      ...
      if (variable.type === "number" && isNaN(variable.value)) {
        this.log("debug", "createConfiguration", "FINISHED");
        throw new HttpErrors.BadRequest(
          "Invalid variable: value is not a number"
        );
      }
    ...
    if (!configObject.draft) {
      ...
      if (
        this.app.versionLimit > 0 &&
        configObject.version > this.app.versionLimit
      ) {
  ...
  async afterSaveHooks(configuration, bases, data) {
    if (configuration.draft !== true) {
      ...
      try {
        await this.app.hookSingleton.executeAdvancedHook(
          "afterCreate",
          "Configuration",
          bases,
          data
        ...
        // Ignore hook errors
        this.log("error", "afterSaveHooks", e.message);
      }
    }
  }
  ...
  async findPromotionCandidates(configuration, options) {
    ...
    bases.forEach((base) => {
      if (
        configuration[base.name] !== undefined &&
        configuration[base.name] !== null
      ) {
    ...
    const candConfig = await this.findWithPermissions(
      { where: whereFilter, order: "effectiveDate DESC" },
      options
    );
  ...
  async findConfigurationForGivenDate(filter, date, options) {
    ...
    const variables = await constVariable.findForDate(
      filter,
      givenDate,
      options
    );
  ...
  async findVariable(search, valueOrName, isRegex, options) {
    ...
    const cursor = await this.app.dataSources["mongoDB"].connector
      .collection("configuration")
      .aggregate(filter, { allowDiskUse: true });
    ...
    const constCursor = await this.app.dataSources["mongoDB"].connector
      .collection("constantVariable")
      .aggregate(filter, { allowDiskUse: true });
