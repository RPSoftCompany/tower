...
module.exports = class Member {
  ...
  async createCache() {
    ...
    const changeStream = this.app.dataSources["mongoDB"].connector
      .collection("Role")
      .watch();
    ...
    const groupChangeStream = this.app.dataSources["mongoDB"].connector
      .collection("group")
      .watch();
  ...
  async login(credentials, include, ttl = 86400) {
    ...
    if (this.ldapServer && credentials.username !== "admin" && !foundUser) {
      try {
        ...
        const password = configuration.decryptPassword(
          this.ldapServer.bindCredentials
        );
      ...
      if (user === null) {
        user = await User.create({
          ...
          newUser: false,
          dn: credentials.dn,
          groups: this.ldapServer.defaultGroups
            ? this.ldapServer.defaultGroups
            : [],
  ...
  async invalidateTokens() {
    await this.app.dataSources["mongoDB"].connector
      ...
      .forEach((doc) => {
        this.app.dataSources["mongoDB"].connector
          .collection("AccessToken")
  ...
  async getUserRoles(userId) {
    ...
    if (
      this.userRolesCache.has(userId.toString()) &&
      this.cacheEnabled &&
      !this.refreshCache
    ) {
