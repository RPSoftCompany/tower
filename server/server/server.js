...
if (process.env.NODE_ENV === "production") {
  ...
  mainConfig.config.privateKey =
    userMainConfig.privateKey || mainConfig.config.privateKey;
  mainConfig.config.certificate =
    userMainConfig.certificate || mainConfig.config.certificate;
...
const versionLimit =
  mainConfig.config.versionLimit !== undefined
    ? mainConfig.config.versionLimit
    : -1;
app.versionLimit = versionLimit;

let nonSafe =
  mainConfig.config.nonSafe === undefined ? false : mainConfig.config.nonSafe;
...
app.fullEncryption =
  mainConfig.config.fullEncryption === undefined
    ? false
    : mainConfig.config.fullEncryption;

const tokenHeaders =
  mainConfig.config.tokenHeaders === undefined
    ? []
    : mainConfig.config.tokenHeaders;

const auditTTL =
  mainConfig.config.auditTTL === undefined ? 20 : mainConfig.config.auditTTL;
...
app.start = () => {
  ...
  if (mainConfig.config.privateKey && mainConfig.config.certificate) {
    const privateKey = fs
      .readFileSync(
        path.join(path.dirname(process.execPath), mainConfig.config.privateKey)
      )
      .toString();
    const certificate = fs
      .readFileSync(
        path.join(path.dirname(process.execPath), mainConfig.config.certificate)
      )
      .toString();
    const options = {
      key: privateKey,
