	...
	<div class="tw-flex tw-flex-col tw-h-full">
		...
		>
			<div class="tw-flex-grow">
				<tower-select
					v-model="currentConnectionClone.region"
					:options="allRegions"
					label="Region"
					:rules="[(val) => !!val || 'Region can\'t be empty']"
				/>
				<div class="tw-flex-grow">
					<q-input
						v-model="currentConnectionClone.accessKeyId"
						dense
						color="secondary"
						:rules="[(val) => !!val || 'Access key ID can\'t be empty']"
						label="Access key ID"
					/>
				</div>
				<div class="tw-flex-grow">
					<q-input
						v-model="currentConnectionClone.secretAccessKey"
						dense
						color="secondary"
...
// Data
//====================================================
const currentConnection: Ref<AWSConnection | null> = ref(null);
const currentConnectionClone: Ref<AWSConnection | null> = ref(null);
const currentConnectionItem: Ref<AWSConnectionItem | null> = ref(null);
const allConnections: Ref<Array<AWSConnection>> = ref([]);
...
const getAllConnections = async () => {
	...
	try {
		...
		if (response?.status === 200 && response.data.length > 0) {
			response.data.forEach((connection: AWSConnection) => {
				const tempConnection = {
					...connection,
					connectionName: `${connection.region} - ${connection.accessKeyId}`,
				};
...
const addConnectionItem = () => {
	if (currentConnectionClone.value) {
		const temp: AWSConnectionItem = { _id: uuidv4() };
 ...
 */
const setConnectionItemName = (item: AWSConnectionItem) => {
	let _name = '';
...
const saveConnection = async () => {
	if (currentConnectionClone.value) {
		...
		currentConnectionClone.value.items.forEach((item) => {
			const tempItem: AWSConnectionItem = {};
			for (const base of allBases.value) {
				tempItem[base.name] = item[base.name];
			}
		...
		try {
			...
			if (response.status === 200) {
				...
				currentConnection.value.connectionName = `${currentConnection.value.region} - ${currentConnection.value.accessKeyId}`;
...
const testConnection = async () => {
	try {
		...
		const response = await towerAxios.post(
			'/connections/testConnection?type=AWSConnection',
			currentConnectionClone.value
		);
