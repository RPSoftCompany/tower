describe('Login', () => {
    ...
    it('Add new config', () => {
        cy.get('[data-cy=configurationPane]').should('be.visible')

        cy.get('[data-cy=newConfigName]').type('new')
        cy.get('[data-cy=newConfigString]').type('string')
        cy.get('[data-cy=newConfigAdd_new]').click()

        cy.get('.newConfigParent > form').should('have.length', 1)
        ...
        cy.get('.newConfigParent > form').should('have.length', 2)
        ...
        cy.get('.newConfigParent > form').should('have.length', 3)
        ...
        cy.get('.newConfigParent > form').should('have.length', 4)
        ...
        cy.get('.newConfigParent > form').should('have.length', 5)
        ...
        cy.get('.newConfigParent > form').should('have.length', 6)
    })
    it('Validate added values', () => {
        cy.get('[data-cy=configurationEditMode]').click()

        cy.get('[data-cy=configRow_new]').should('have.value', 'string')
        cy.get('[data-cy=configRow_new_number]').should('have.value', '1')
        cy.get('[data-cy=configRow_new_password]').should('have.value', 'password')
        cy.get('[data-cy=configRow_new_boolean][aria-checked=true]').should('have.length', 1)
        cy.get('[data-cy=configRow_new_text]').should('have.value', 'text')
